...
if (window.L) {
  const map = L.map('map', {
>>>>>>> d884e1b07bd10fa48b8517c107a8a2cecc319f51
    zoomControl: false,
    dragging: true,
...
=======
  ...
  // Kedvencek (localStorage)
  const FAV_KEY = 'dma_favorites';
  function favKeyOf(p) { return `${p.name}|${p.coordinates[0]},${p.coordinates[1]}`; }
  function readFavs() {
    try { return new Set(JSON.parse(localStorage.getItem(FAV_KEY)) || []); } catch { return new Set(); }
  }
  function writeFavs(set) { localStorage.setItem(FAV_KEY, JSON.stringify(Array.from(set))); }
  function isFav(p, favs) { return favs.has(favKeyOf(p)); }
  function toggleFav(p) {
    const favs = readFavs();
    const k = favKeyOf(p);
  ...
  function updateFavCount() {
    const c = document.getElementById('fav-count');
    if (!c) return;
    try { const n = JSON.parse(localStorage.getItem(FAV_KEY) || '[]').length; c.textContent = n ? `(${n})` : ''; } catch { c.textContent = ''; }
>>>>>>> d884e1b07bd10fa48b8517c107a8a2cecc319f51
  }
...
=======
    ...
    const filtered = currentTab === 'favs'
      ? list.filter(p => isFav(p, favs))
      : list;

    filtered.forEach((p) => {
      const li = document.createElement('li');
      li.className = 'p-3 rounded-lg bg-white/5 border border-white/10 hover:border-[#a855f7]/60 hover:shadow-[0_0_14px_#a855f7] transition';
      const heartFilled = isFav(p, favs);
      li.innerHTML = `<div class="flex items-center justify-between gap-2">
        <div class="min-w-0">
          <div class="font-semibold truncate">${p.name}</div>
        ...
        <button class="fav-btn shrink-0 w-8 h-8 rounded-full border border-white/10 bg-white/5 hover:bg-white/10" title="Kedvenc">
          <span class="block text-${heartFilled?'[#a855f7]':'white/70'}">${heartFilled?'❤':'♡'}</span>
        </button>
      </div>`;
  ...
  function setTab(tab) {
    ...
    // rebuild list using lastPlaces
    const withDist = lastPlaces.map(p => ({...p, distance: distanceKm(lastOrigin, p.coordinates)}))
      .sort((a,b)=>a.distance-b.distance);
    renderList(withDist);
  }
  if (tabNearby) tabNearby.addEventListener('click', () => setTab('nearby'));
